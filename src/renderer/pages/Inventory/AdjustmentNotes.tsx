...
import { Table, Button, Space, Modal, Form, Input, DatePicker, Select, InputNumber, message, notification, Tag } from 'antd';
    ...
    const loadNotes = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                const filtered = (result.data || []).filter((n: any) => String(n?.adjustment_number || '').includes(`/${yy}`));
                setNotes(filtered);
            }
        } catch (error) {
    ...
    const handleNewNote = async () => {
        ...
        if (currentCompany) {
            try {
                ...
                form.setFieldsValue({
                    adjustment_number: result.data || result, // API might return just the string or { success: true, data: string }
                    adjustment_date: dayjs(),
                    adjustment_type: 'IN'
                });
            } catch (err) {
                console.error('Failed to load next note number:', err);
                form.setFieldsValue({ adjustment_date: dayjs(), adjustment_type: 'IN' });
            }
        }
    };
            ...
            >
                <Form form={form} layout="vertical" onFinish={handleSave}>
                    <Space align="start" wrap>
                        ...
                        <Form.Item name="adjustment_type" label="Adjustment Type" rules={[{ required: true }]} style={{ width: 160 }}>
                             <Select>
                                 <Select.Option value="IN">Adjustment IN</Select.Option>
                                 <Select.Option value="OUT">Adjustment OUT</Select.Option>
                             </Select>
                        </Form.Item>
